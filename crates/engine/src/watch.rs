...
pub struct WatchRegistry {
    keys: FxHashMap<(Bytes, Bytes), Vec<UnboundedSender<WatchEvent>>>,
    prefixes: Vec<((Bytes, Bytes), UnboundedSender<WatchEvent>)>,
}
...
impl WatchRegistry {
    ...
    pub fn subscribe_key(&mut self, ns: Bytes, key: Bytes) -> UnboundedReceiver<WatchEvent> {
        let (tx, rx) = unbounded();
        self.keys.entry((ns, key)).or_default().push(tx);
        rx
    }

    pub fn subscribe_prefix(&mut self, ns: Bytes, prefix: Bytes) -> UnboundedReceiver<WatchEvent> {
        let (tx, rx) = unbounded();
    ...
    pub fn notify(&mut self, ns: &str, key: &[u8], event: WatchEvent) {
        ...
        if let Some(senders) = self.keys.get_mut(&(ns_b.clone(), key_b.clone())) {
            ...
            if senders.is_empty() {
                self.keys.remove(&(ns_b.clone(), key_b));
            }
        }

        self.prefixes.retain(|((wns, prefix), tx)| {
            if wns == &ns_b && key.starts_with(prefix.as_ref()) {
...
mod tests {
    ...
    fn prefix_receives_matching_keys() {
        ...
        assert_eq!(rx.try_recv().unwrap_err(), TryRecvError::Empty); // other filtered
    }
